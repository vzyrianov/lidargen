###############################################################################
#
# Retrieved from https://github.com/lrjconan/GRAN/blob/43cb4433e6f69401c3a4a6e946ea75da6ec35d72/utils/dist_helper.py#L118
#
###############################################################################
...
def gaussian_dist(x, y, sigma=180.3):
  support_size = max(len(x), len(y))
  # convert histogram values x and y to float, and make them equal len
  x = x.astype(np.float)
  y = y.astype(np.float)
...
def compute_mmd_sigma(samples1, samples2, kernel, is_hist=True, *args, **kwargs):
  ''' Empirical Sigma Estimate '''
  # normalize histograms into pmf  
  if is_hist:
    samples1 = [s1 / np.sum(s1) for s1 in samples1]
    samples2 = [s2 / np.sum(s2) for s2 in samples2]
  return calc_sigma(samples1, samples2, gaussian_dist, *args, **kwargs)