...
def conv3x3(in_planes, out_planes, stride=1, bias=True, spec_norm=False):
    ...
    conv = nn.Conv2d(in_planes, out_planes, kernel_size=3, stride=stride,
                     padding=1, bias=bias, padding_mode="circular")
    if spec_norm:
        conv = spectral_norm(conv)
...
def stride_conv3x3(in_planes, out_planes, kernel_size, bias=True, spec_norm=False):
    conv = nn.Conv2d(in_planes, out_planes, kernel_size=kernel_size, stride=2,
                     padding=kernel_size // 2, bias=bias, padding_mode="circular")
    if spec_norm:
        conv = spectral_norm(conv)
    return conv
...
def dilated_conv3x3(in_planes, out_planes, dilation, bias=True, spec_norm=False):
    conv = nn.Conv2d(in_planes, out_planes, kernel_size=3, padding=dilation, dilation=dilation, bias=bias, padding_mode="circular")
    if spec_norm:
        conv = spectral_norm(conv)
...
class UpsampleConv(nn.Module):
    def __init__(self, input_dim, output_dim, kernel_size=3, biases=True, spec_norm=False):
        super().__init__()
        self.conv = nn.Conv2d(input_dim, output_dim, kernel_size, stride=1, padding=kernel_size // 2, bias=biases, padding_mode="circular")
        if spec_norm:
            self.conv = spectral_norm(self.conv)
